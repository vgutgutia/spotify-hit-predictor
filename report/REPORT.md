...
The full pipeline goes from raw Kaggle CSVs to a deployed, interactive Streamlit web app on Hugging Face Spaces. A user can either drag sliders to design a hypothetical song and watch the prediction update live, or paste any Spotify track URL and have the app fetch that song's real audio features through the Spotify Web API and run them through the model. Both modes show a hit/flop verdict, a hit probability, and a SHAP-based breakdown of which features pushed *this specific song* toward or away from being a hit.
...
6. **The Spotify audio-features API was deprecated for new developer apps in November 2024.** Spotify restricted `/v1/audio-features` and `/v1/audio-analysis` to apps with "extended quota mode" access — which new student-tier apps don't get. As a result, the Spotify URL tab of the app returns a 403 unless the deploying user happens to own a grandfathered app. The Sliders tab is unaffected. This is purely a Spotify API policy change, not a model or code issue, but it does mean the "paste a URL, get a prediction" use case in my original proposal is no longer fully reproducible from a fresh developer account. If I rebuilt this today, I'd either compute the audio features locally from the song's 30-second preview MP3 using `librosa` (matching the dataset's features as closely as possible), or use a community alternative like ReccoBeats that reverse-engineers comparable features.
...
The thing that surprised me most was how much harder the "wrap it as a real product" half of the project was than the "train a model" half. Training was a quiet hour of `scikit-learn` and `xgboost` defaults that produced ~89% ROC-AUC on the first try with no tuning. Building the Streamlit app — getting the Spotify URL parser to handle every URL shape, hooking up the Spotify API with the right OAuth flow, making sure the feature vector at inference time matches the column order the model was trained on, handling the case where the secrets file doesn't exist yet — took longer than everything else combined.
